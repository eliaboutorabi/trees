...
export function createGroundMaterial() {
  ...
  // This shader covers the entire screen, so every octave here is paid for at
  // full resolution on every pixel. Two cheap layers is enough now that the
  // terrain carries real relief and the scattered tufts carry the fine detail —
  // a third high-frequency layer was costing more than it showed.
  const broad = mx_fractal_noise_float(vec3(p.x.mul(0.16), 0, p.z.mul(0.16)), 3, 2.0, 0.55, 1.0).mul(0.5).add(0.5);
  const fine = mx_fractal_noise_float(vec3(p.x.mul(1.6), 0, p.z.mul(1.6)), 2, 2.0, 0.5, 1.0).mul(0.5).add(0.5);

  const grass = mix(uniforms.near, uniforms.far, broad.mul(0.72).add(fine.mul(0.28)));
  ...
  material.roughnessNode = float(0.97).sub(fine.mul(0.09));
